[System.Serializable]
public class DataSave
{
    public ulong suses;
    public uint multiplier;
    public ulong multiplierCost;
    public uint Callums;
    public uint Taylors;
    public uint Greens;
    public float GreenCost;
    public float CallumCost;
    public float TaylorCost;
    public uint Nathaniels;
    public float NathanielCost;
    public float FloppaCost;
    public uint Floppas;
    public float BingusCost;
    public uint Bingus;
    public float SoggaCost;
    public uint Soggas;
    public float SaulCost;
    public uint Sauls;
    public float JesseCost;
    public uint Jesses;
    public float WalterCost;
    public uint Walters;
    public DataSave(ulong suses, uint multiplier, 
    ulong multiplierCost,  uint Greens, uint Callums, uint Taylors, float GreenCost,
    float CallumCost, float TaylorCost, uint Nathaniels, float NathanielCost, float FloppaCost,
    uint Floppas, float BingusCost, uint Bingus, float SoggaCost, uint Soggas, float SaulCost,
    uint Sauls, float JesseCost, uint Jesses, float WalterCost, uint Walters)
    {
        this.suses = suses;
        this.multiplier = multiplier;
        this.multiplierCost = multiplierCost;
        this.Greens = Greens;
        this.Callums = Callums;
        this.Taylors = Taylors;
        this.GreenCost = GreenCost;
        this.CallumCost = CallumCost;
        this.TaylorCost = TaylorCost;
        this.Nathaniels = Nathaniels;
        this.NathanielCost = NathanielCost;
        this.FloppaCost = FloppaCost;
        this.Floppas = Floppas;
        this.BingusCost = BingusCost;
        this.Bingus = Bingus;
        this.SoggaCost = SoggaCost;
        this.Soggas = Soggas;
        this.SaulCost = SaulCost;
        this.Sauls = Sauls;
        this.JesseCost = JesseCost;
        this.Jesses = Jesses;
        this.WalterCost = WalterCost;
        this.Walters = Walters;
    }
}